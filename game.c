...
bool LoadAssets(){
    if((background = loadBitmap("Background.bmp")) == NULL) return false;

    if((GreenWizard = CreateSprite("Green_Hat.bmp")) == NULL) return false;
...
Sprite* CreateSprite(char img_name[]){
    Bitmap* bmp;
    if((bmp = loadBitmap(img_name)) == NULL) return NULL;
...
Wizard* CreateWizard(enum Wizard_color color, int center_x, int center_y, unsigned int rot){
    ...
    switch(color){
        case Red: break;
        case Green: wizard->img = GreenWizard; break;
        case Blue: break;
        case Yellow: break;
    }
